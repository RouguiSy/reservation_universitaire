...
class ReservationValidator implements ValidatorInterface
{
    ...
    {
        $erreurs = [];
        $salleId = filter_var($data['salle_id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        if ($salleId === false || !v::intType()->positive()->validate($salleId)) $erreurs['salle_id'] = "L'identifiant est invalide.";
        ...
        if ($debut === null) $erreurs['date_debut'] = 'La date de debut est invalide.';
        if ($fin === null) $erreurs['date_fin'] = 'La date de fin est invalide.';
    ...
    {
        if (!is_string($value) || !v::stringType()->notEmpty()->validate($value)) {
            return null;
        }
